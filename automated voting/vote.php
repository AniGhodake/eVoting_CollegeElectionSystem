...
<body>
	...
	<div class="col-lg-6">
				...
				<div class="col-lg-6">
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading"><center>
							UNIVERSITY REPRESENTATIVE (UR)</center>
                        </div>
                        <div class="panel-body" style = "background-color:;">
						<?php
							$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'UNIVERSITY REPRESENTATIVE (UR)'") or die(mysqli_errno());
							while($fetch = $query->fetch_array()){
						?>
		<div id = "position"><center>
			<img class = "image-rounded" src = "admin/<?php echo $fetch['img']?>"style ="border-radius:6px;" height = "150px" width = "150px"></center>
		<center><button type="button" class="btn btn-primary btn-xs" style = "border-radius:60px;margin-top:4px;"><?php echo $fetch['firstname']." ".$fetch['lastname']?></button></center>
			<center><input type = "radio" value = "<?php echo $fetch['candidate_id'] ?>" name = "ur_id" class = "ur"></center>
		</div>
						<?php
							}
						?>
